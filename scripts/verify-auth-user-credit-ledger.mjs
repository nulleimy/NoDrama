...
if (ledger.includes("prompt") || ledger.includes("reply")) fail("Ledger should not store full prompt/reply content.");
...
if (!addRoute.includes("NODRAMA_TEST_MODE") && !addRoute.includes("NODRAMA_ALLOW_DEV_CREDIT_GRANTS")) {
...
if (!webhookRoute.includes("source: \"stripe_webhook\"")) {
  fail("Webhook should grant through server trusted source metadata.");
}

const envExample = readFileSync(".env.example", "utf8");
if (!envExample.includes("NODRAMA_ALLOW_DEV_CREDIT_GRANTS=") && !envExample.includes("NODRAMA_ENABLE_DEV_CREDIT_GRANTS=")) {
...
console.log("✅ Auth user credit ledger verified");
