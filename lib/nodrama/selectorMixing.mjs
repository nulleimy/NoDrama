...
import {
  applyReplyIntelligenceRouting,
  detectIntentConflict,
  detectReplyContext,
  normalizeSelectorSources,
} from "./replyIntelligence.mjs";
...
export function createNormalizedGenerationContext(input, category, language = "cs") {
  ...
  const initialStrategy = normalizeSelector({
  ...
  const selectorSources = normalizeSelectorSources(input.selectorSources);
  const detectedContext = detectReplyContext(userText, {
    strategyId: initialStrategy.id,
    relationshipId: relationship.id,
    channelId: channel.id,
  });
  const routed = applyReplyIntelligenceRouting(
    {
      strategyId: initialStrategy.id,
      relationshipId: relationship.id,
      channelId: channel.id,
  ...
  const strategy =
    routed.selected.strategyId === initialStrategy.id
      ? initialStrategy
      : {
          id: routed.selected.strategyId,
          source: initialStrategy.source,
          requestedId: initialStrategy.id,
          adjusted: true,
          adjustmentReason: routed.warnings[0],
        };
  ...
  const inferredDomain = detectedContext.domain;
  const inferredScenarioFamily = detectedContext.scenarioFamily;
  ...
  const compatibility = scoreCompatibility({
    ...
    matchedMicroSituation,
    confidence,
    warnings: [...safetyWarnings, ...detectedContext.warnings, ...routed.warnings],
  });
  const intentConflicts = detectIntentConflict(detectedContext, {
    strategyId: initialStrategy.id,
    relationshipId: relationship.id,
    channelId: channel.id,
  });
  ...
  return {
    ...
    confidence:
      detectedContext.confidence === "high" || confidence === "high"
        ? "high"
        : detectedContext.confidence === "medium" || confidence === "medium"
        ? "medium"
        : "low",
    safetyNotes:
    ...
    safetyWarnings: [
      ...new Set([...safetyWarnings, ...detectedContext.warnings, ...routed.warnings]),
    ],
    signalKeywords,
    replyIntelligence: {
      detectedContext,
      intentConflicts,
      selectedSources: selectorSources,
      routingWarnings: routed.warnings,
    },
