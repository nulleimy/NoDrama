...
const planSku: Record<string, string | null> = {
  Free: null,
...
export function PricingCards({ lang = "cs" }: { lang?: "cs" | "en" }) {
  const paidCtaLabel = lang === "cs" ? "Vybrat plán" : "Choose plan";
          ...
          return (
            ...
            >
              ...
              {sku ? (
                <CheckoutButton
                  sku={sku}
                  eventSource="pricing_plan"
                  className={[
                    "w-full rounded-full px-4 py-3 text-sm font-black transition focus:outline-none focus:ring-4 focus:ring-[#B8FF4D]/35 disabled:cursor-wait disabled:opacity-70",
                    plan.highlighted
